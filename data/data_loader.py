    ...
    for dname in dataset_list:
        ...
        if args.type == 'jigsaw':
            ...
            train_dataset = JigsawDataset(dname, split='train', val_size=args.val_size,
                    ...
                    jig_classes=args.aux_classes, bias_whole_image=args.bias_whole_image, root_dir=root)
            val_dataset = JigsawTestDataset(dname, split='val', val_size=args.val_size,
                img_transformer=get_val_transformer(args), jig_classes=args.aux_classes, root_dir=root)
            ...
            train_dataset = RotateDataset(dname, split='train', val_size=args.val_size,
                    img_transformer=img_transformer, rot_classes=args.aux_classes, bias_whole_image=args.bias_whole_image, root_dir=root)
            val_dataset = RotateTestDataset(dname, split='val', val_size=args.val_size,
                img_transformer=get_val_transformer(args), rot_classes=args.aux_classes, root_dir=root)
...
def get_target_dataloader(args):
    ...
    if args.type == 'jigsaw':
        ...
        dataset = JigsawDataset(name, 'train', img_transformer=img_transformer,
                ...
                bias_whole_image=args.bias_whole_image, root_dir=root)
    elif args.type == 'rotate':
        img_transformer = get_rot_train_transformers(args)
        dataset = RotateDataset(name, 'train', img_transformer=img_transformer,
                rot_classes=args.aux_classes, bias_whole_image=args.bias_whole_image, root_dir=root)
...
def get_test_dataloader(args):
    ...
    if args.type == 'jigsaw':
        val_dataset = JigsawTestDataset(name, split='test',
                img_transformer=img_tr, jig_classes=args.aux_classes, root_dir=root)
    elif args.type == 'rotate':
        val_dataset = RotateTestDataset(name, split='test',
                img_transformer=img_tr, rot_classes=args.aux_classes, root_dir=root)
