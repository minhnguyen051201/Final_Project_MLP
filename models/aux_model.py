...
class AuxModel:
    ...
    def __init__(self, args, logger):
        ...
        # Provide defaults for missing high-level controls
        if not hasattr(args, 'mode'):
            args.mode = 'train'

        # Build a default training config from SOLVER/PRETEXT if missing
        if not hasattr(args, 'training'):
            num_epochs = None
            lr = None
            rot_weight = None
            try:
                num_epochs = getattr(getattr(args, 'SOLVER', EasyDict()), 'EPOCHS', None)
                lr = getattr(getattr(args, 'SOLVER', EasyDict()), 'LR', None)
                rot_weight = getattr(getattr(args, 'PRETEXT', EasyDict()), 'ROT_WEIGHT', None)
            except Exception:
                pass
            ...
            if rot_weight is None:
                rot_weight = 0.5
            scheduler_step = max(int(num_epochs * 0.8), 1)
            args.training = EasyDict({
                'resume': '',
                'num_print_epoch': 10,
                'num_epochs': int(num_epochs),
                'optimizer': EasyDict({'name': 'sgd', 'lr': float(lr), 'weight_decay': 5e-4, 'momentum': 0.9, 'nesterov': True}),
                'lr_scheduler': EasyDict({'name': 'step', 'step_size': scheduler_step}),
                'src_aux_weight': float(rot_weight),
                'tar_aux_weight': float(rot_weight),
        ...
        # Resolve network architecture from either legacy or new config paths
        arch = None
        try:
            arch = args.network.arch
        except Exception:
            pass
        if arch is None:
            try:
                ...
                arch = 'resnet50'  # sensible default

        # Ensure aux_classes (number of rotation classes excluding the unrotated one)
        if not hasattr(args, 'aux_classes'):
            aux_classes = None
            try:
                angles = getattr(getattr(args, 'PRETEXT', EasyDict()), 'ANGLES', None)
                if angles and isinstance(angles, (list, tuple)) and len(angles) >= 2:
                    # With [0,90,180,270], aux_classes should be 3 (exclude 0)
                    aux_classes = max(len(angles) - 1, 1)
            except Exception:
                pass
            if aux_classes is None:
                aux_classes = 3
            args.aux_classes = aux_classes

        # Ensure n_classes (number of semantic classes)
        if not hasattr(args, 'n_classes'):
            n_classes = None
            try:
                dataset_name = str(getattr(getattr(args, 'DATA', EasyDict()), 'NAME', '')).lower()
                # Common defaults for known datasets
        ...
        self.model = get_aux_net(str(arch).lower())(aux_classes=args.aux_classes + 1, classes=args.n_classes)
